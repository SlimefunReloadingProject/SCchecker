...
def report(i):
    global config, totalBug
    totalBug += 1
    if totalBug == config['MaxPrintBug']:
        error(f"[BUG]{totalBug}. 在 {i}:", end="\n  ")
        error(f"[BUG] Bug数量已达到上限！请修复以上Bug再运行本程序！")
    elif totalBug < config['MaxPrintBug']:
        error(f"[BUG]{totalBug}. 在 {i}:", end="\n  ")

def printc(string):
    print(f'{color.gold}{string}')
...
def isAmountProper(item, dAm, position):
    stack = getItemMaxStack(item) if inVanilla(str(item)) else 64
    isInt(dAm, f'{position} ', 1, stack)
...
def TIA(dat, position):
    ...
    if dType != 'NONE':
        isAmountProper(str(dId).upper(), dAm, position+'amount')
...
def isRunningRecipeProper(data, position):
    ...
    for j in recipes:
        ...
        if len(Input) != 2:
            ...
            error('input参数数量错误')
        if Input['1']['id'] == Input['2']['id'] and Input['1']['amount'] == 64 and Input['2']['amount'] != 0:
            report(position+f'第 {j} 个配方的 input')
            error('amount总和 > 64，这会导致一些bug！')
        for k in Input:
            TIA(Input[k], position+f'第 {j} 个配方的 input 的第 {k} 个物品的 ')
        if len(Output) != 2:
            report(position+f'第 {j} 个配方的 output')
            error('output参数数量错误')
        for k in Output:
            TIA(Output[k], position+f'第 {j} 个配方的 output 的第 {k} 个物品的 ')
...
def checkScAddon():
    printc('Testing sc-addon')
    with open('sc-addon.yml', 'r', encoding='utf-8') as file:
...
def checkMobDrops():
    ...
    for i in k:
        ...
        isAmountProper('PLACEHOLDER', dAm, f'mob-drops: {i} 的 item-amount')
        dChance = data['chance']
...
def checkGeoResources():
    ...
    for i in k:
        ...
        if not ok:
            report(i)
            error(f"{i} 未找到 biome 或 environment")
        items.add(i)

def checkItems():
    printc("Testing items")
    ...
    for i in k:
        ...
        dAm = data['item-amount']
        isAmountProper(data['item-id'], dAm, f'items: {i} 的 item-amount')
...
def checkCapacitors():
    ...
    for i in k:
        ...
        if dType not in ('DEFAULT','default'):
            ...
            isAmountProper(dType, dAm, f'capacitors: {i} 的 item-amount')
        dCapa = data['capacity']
...
def checkSolarGenerators():
    printc("Testing solar-generators")
...
def checkMaterialGenerators():
    printc("Testing material-generators.yml")
...
def checkResearches():
    ...
    for i in k:
        ...
        for j in data['items']:
            isSlimefun(j, "researches: {i} 的 items")

try:
    sum_start = time()
    print(f'{color.gold} Loading config')
    ...
    for i in loadedItems:
        keys.append(tuple(i.keys())[0])
        values.append(tuple(i.values())[0])
